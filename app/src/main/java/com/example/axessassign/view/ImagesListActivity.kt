...
import com.example.axessassign.adapter.ImageGridAdapter
import com.example.axessassign.constants.AppConstants
import com.example.axessassign.model.Data
import com.example.axessassign.model.Images
import com.example.axessassign.utility.AppUtility
import com.example.axessassign.viewmodel.ImagesViewModel
...
class ImagesListActivity : AppCompatActivity(), ImageGridAdapter.UserItemClickListener, SearchView.OnQueryTextListener, SearchView.OnCloseListener {
    ...
    override fun onQueryTextChange(query: String?): Boolean {
        ...
        if(query.isNullOrBlank()){
            ...
            if (AppUtility.isNetworkAvailable(this@ImagesListActivity)) {
                queryTextChangedJob = coroutineScope.launch {
                    delay(250)
                    searchString = query
                    toolbar_title.text = searchString.capitalize()
                    pageCount = 1
                    loadDefaultList(query)
                }
            } else {
                AppUtility.showSnackBar(root_view_movie_list, getString(R.string.check_internet_connection))
            }
        }
    ...
    override fun onClose(): Boolean {
        AppUtility.hideKeyboardFrom(this, root_view_movie_list)
        pageCount = 1
        return false
    }
    ...
    private fun setDataToRecyclerView(it: Images) = if (loadMore) {
        ...
        adapter = ImageGridAdapter(this, this, userList)
    ...
    override fun onUserItemClicked(userModel: Data, position: Int) {
        ...
        overridePendingTransition(R.anim.anim_slide_in_left, R.anim.right_to_left)
